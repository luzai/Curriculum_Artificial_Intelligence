...
class LeNet:
    ...
    def train(self):
        ...
        for i in range(30001):
            ...
            if i % 5 == 0:
                [train_accuracy, s,loss] = self.sess.run([self.accuracy, self.summ,self.loss],
            ...
            if i % 500 == 0:
                [test_acc,s,loss] = self.sess.run([self.accuracy,self.summ,self.loss],
                                           feed_dict={self.x: mnist.test.images, self.y: mnist.test.labels,
                                                      self.keep_prob: 1})
                test_writer.add_summary(s,i)
                saver.save(self.sess, 'log/model.ckpt', i)
                print "test loss {} acc {}".format(test_acc,loss)
